  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        {/* Sent list */}
        <div className={`col-span-12 lg:col-span-4 border overflow-hidden flex flex-col h-[680px] ${card} ${viewingDetail ? 'hidden lg:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div
                ...
                className={`p-4 cursor-pointer transition-colors border-b border-black ${row} ${selectedMessage?.id === msg.id ? rowSel : ''}`}
              >
                ...
                <div className="mt-2 flex items-center gap-1.5 flex-wrap">
                  <span className={`text-[10px] px-2 py-0.5 border ${isLight ? 'bg-[#f3f4f6] border-[#e5e7eb] text-[#6b7280]' : 'bg-[#26262d] border-[#3a3a40] text-zinc-400'}`}>
                    {msg.recipient_usn}
                  </span>
                  {msg.is_read ? (
                    <span className="text-[10px] px-2 py-0.5 border border-emerald-500/30 bg-emerald-500/10 text-emerald-400">✓ Read</span>
                  ) : (
                    <span className="text-[10px] px-2 py-0.5 border border-amber-500/30 bg-amber-500/10 text-amber-400">Unread</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {/* Detail panel */}
        <div className={`col-span-12 lg:col-span-8 border h-[680px] flex flex-col ${card}`}>
          {selectedMessage ? (
            <>
              <div className={`px-6 py-4 border-b flex items-start justify-between gap-4 ${divider}`}>
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  {viewingDetail && (
                    <button
                      ...
                      className={`mt-1 h-8 w-8 flex items-center justify-center transition-colors lg:hidden flex-shrink-0 ${isLight ? 'hover:bg-[#f3f4f6] text-[#6b7280]' : 'hover:bg-[#2a2a2e] text-zinc-400'}`}
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div className="h-8 w-8 bg-[#f7b545]/20 flex items-center justify-center text-[#f7b545] font-bold text-xs shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {selectedMessage.message_type === 'Official' && (
                    <span className={`text-[10px] px-2 py-1 border font-semibold tracking-wide ${isLight ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-blue-500/10 border-blue-500/30 text-blue-400'}`}>
                      OFFICIAL
                    </span>
                  )}
                  {selectedMessage.is_read ? (
                    <span className="text-[10px] px-2 py-1 border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 font-semibold">
                      ✓ READ
                    </span>
                  ) : (
                    <span className="text-[10px] px-2 py-1 border border-amber-500/30 bg-amber-500/10 text-amber-400 font-semibold">
                      PENDING
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3">
              <div className={`h-16 w-16 flex items-center justify-center ${isLight ? 'bg-[#f3f4f6]' : 'bg-[#26262d]'}`}>
      ...
      {showComposeModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
          ...
          <div className={`relative w-full max-w-xl border shadow-2xl overflow-hidden ${modalBg}`}>
            ...
            <form onSubmit={handleSendMessage} className="px-6 py-5 space-y-4">
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    type="text"
                    className={`w-full h-10 pl-9 pr-20 border text-sm outline-none transition-colors ${inp}`}
                  ...
                  {selectedStudent && !isSearchingStudents && (
                    <button
                      ...
                      className={`absolute right-2 top-1/2 -translate-y-1/2 text-[10px] px-2 py-0.5 border font-semibold transition-colors ${isLight ? 'border-[#d1d5db] hover:bg-[#f3f4f6] text-[#374151]' : 'border-[#3a3a40] hover:bg-[#2a2a2e] text-zinc-300'}`}
                    >
                      Change
                    </button>
                  )}
                ...
                {!selectedStudent && studentResults.length > 0 && (
                  <div className={`absolute z-10 w-full mt-1 border shadow-xl overflow-hidden ${modalBg} ${divider}`}>
              ...
              <div className={`flex items-center justify-between pt-3 border-t ${divider}`}>
                <span className="text-[10px] px-2 py-1 border border-amber-500/30 bg-amber-500/10 text-amber-400 font-semibold tracking-wide">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={closeModal}
                    disabled={sending}
                    className={`px-4 py-2 text-sm font-semibold border transition-colors ${isLight ? 'border-[#d1d5db] hover:bg-[#f3f4f6] text-[#374151]' : 'border-[#3a3a40] hover:bg-[#2a2a2e] text-zinc-300'}`}
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={sending || !selectedStudent}
                    className="flex items-center gap-2 px-5 py-2 text-sm font-bold bg-[#f7b545] hover:bg-[#e5a530] text-[#1a1a1e] transition-colors disabled:opacity-50"
                  >
                    {sending ? (
